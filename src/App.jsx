      ...
      <div style={{maxWidth: '900px', margin: '0 auto', padding:'12px 16px 0'}}>
        <div style={{
          ...
          <div style={{display:'flex', alignItems:'center', justifyContent:'space-between', gap:10, flexWrap:'wrap'}}>
            ...
            <div className="badge" style={{fontSize:12, color:'#cfcfcf'}}>{dateStr}</div>
          </div>
        </div>
        <div style={{display:'flex', alignItems:'center', justifyContent:'flex-end', paddingTop:8}}>
          <div className="badge" style={{fontSize:12, color:'#cfcfcf'}}>⭐ Star Seeds: {seeds}</div>
        </div>
      </div>

      {/* Main card */}
      <div style={{maxWidth: '900px', margin:'0 auto', padding:'0 16px'}}>
        <div className="card" style={{background:'#121212', border:'1px solid #2a2a2a', borderRadius:22, padding:16}}>
          ...
          <div style={{display:'flex', flexDirection:'column', alignItems:'center', paddingTop:'8px'}}>
            ...
            <div style={{marginTop:'16px', display:'flex', flexWrap:'wrap', gap:'10px', justifyContent:'center'}}>
              <button className="btn" style={btnPrimary} onClick={()=>setNightOpen(true)}>🌙 Night Check-in</button>
              <button className="btn" style={btnSecondary} onClick={()=>setMorningOpen(true)}>☀️ Morning Habits</button>
            </div>

            {/* Quantum Card */}
            <div style={{background:'#121212', border:'1px solid #2a2a2a', borderRadius:16, padding:12, width:'100%', marginTop:16}}>
        ...
        {nightOpen && (
          <motion.div className="modal-backdrop" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}
            ...
            <motion.div initial={{y:40, opacity:0}} animate={{y:0, opacity:1}} exit={{y:10, opacity:0}} style={modal}>
              ...
              <div style={{fontSize:12, letterSpacing:1, textTransform:'uppercase', color:'#bdbdbd'}}>Track</div>
              <div style={{display:'grid', gridTemplateColumns:'repeat(2,1fr)', gap:8, marginTop:6}}>
                {tracks.map(t => <button key={t} style={pill} onClick={()=>{}}>{t}</button>)}
              </div>
              <div style={{height:8}} />
              <div style={{fontSize:12, letterSpacing:1, textTransform:'uppercase', color:'#bdbdbd'}}>Mood</div>
              ...
              <div style={{height:8}} />
              <div style={{fontSize:12, letterSpacing:1, textTransform:'uppercase', color:'#bdbdbd'}}>Claim Word</div>
              <input value={claim} onChange={e=>setClaim(e.target.value)} placeholder="Type a word from tonight’s meditation"
                     style={input} />
...
const cardSimple = { background:'#121212', border:'1px solid #2a2a2a', borderRadius:16, padding:12, marginTop:16 }
